...
#dada2 QC parameters:Position at which forward/reverse read sequences should be truncated due to decrease in quality, need to be adjusted according to inspection of Interactive Quality Plot tab in the demux_trim.qzv 
ptf=250
ptr=207
...
--i-demultiplexed-seqs ${ResultDir}/demux_trim.qza \
--p-trunc-len-f $ptf \
--p-trunc-len-r $ptr \
--p-n-threads $Nproc \
...
--o-classification ${SCRATCH}/taxonomy.qza --p-n-jobs 3
